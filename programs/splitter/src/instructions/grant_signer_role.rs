...
pub fn handle_grant_signer_role(ctx: Context<GrantSignerRole>, signer: [u8; 64]) -> Result<()> {
    ...
    require!(
        !ctx.accounts.admin.key().eq(&Pubkey::new_from_array(signer[..32].try_into().unwrap())),
        ErrorCode::AdminEqualsSigner
    );
    require!(!config.pauser.eq(&Pubkey::new_from_array(signer[..32].try_into().unwrap())), ErrorCode::PauserEqualsSigner);
