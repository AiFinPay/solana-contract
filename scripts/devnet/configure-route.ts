...
// ---------------------------------------------------------------------------
async function main() {
  console.log("=== AiFinPay Splitter v1.4 — Configure Route ===\n");

  // --- Load admin keypair (configure_route requires admin signer) ---
  const adminKeypairPath = process.env.ADMIN_KEYPAIR_PATH || "keypairs/admin.json";
  ...
  console.log("Done! Run 'npm run check:devnet' to verify.");
}
