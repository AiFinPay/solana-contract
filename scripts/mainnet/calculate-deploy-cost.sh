#!/usr/bin/env bash
# Deploy cost calculator for an Anchor/Solana program (MAINNET).
# Mirror of scripts/devnet/calculate-deploy-cost.sh pointed at mainnet-beta.
# Queries the cluster for rent-exempt minimums and estimates total
# deployment cost including program data account rent, transaction fees, and
# PDA rent. Read-only: spends no SOL.
#
# Usage:
...
log "=== ${PROGRAM_NAME} Deploy Cost Calculation (MAINNET) ==="
...
log "  • Mainnet fees may exceed this estimate under congestion; add a margin."
